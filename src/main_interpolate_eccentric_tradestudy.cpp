...
outputs run_trade_study_bootstrap(double alt, double d, double e, bool hot_finish, bool DisableBootstrap)
{
...
outputs run_trade_study_interpolate(double alt, double d, double e)
{
    // satellite properties
    double mass = 212;        // sat mass (kg)
    double area = 10;         // sat wetted area (m^2)
...
int main(int argc, char **argv)
{
    ...
    double alt = 120;
    vector<double> es = {0.0,0.2,0.4,0.6,0.8};
    ...
    map<pair<double, double>, outputs> DataNoBootstrap;   
    int counter = 0;
    // run_trade_study_bootstrap(120, 0, 0.2, false, true);
    run_trade_study_interpolate(480, 0, 0.2);
    // printProgress(0.0);
    // for (double e : es)
    // {
    ...
    //         outputs output = run_trade_study_bootstrap(alt, d, e, false, false);
    //         DataBootstrap.insert(make_pair(make_pair(e, d), output));
    //         output = run_trade_study_interpolate(alt, d, e);
    //         DataInterpolate.insert(make_pair(make_pair(e, d), output));
    //         output = run_trade_study_bootstrap(alt, d, e, true, false);
    //         DataFinish.insert(make_pair(make_pair(e, d), output));
    //         output = run_trade_study_bootstrap(alt, d, e, false, true);
    //         DataNoBootstrap.insert(make_pair(make_pair(e, d), output));
    //         counter++;
    //         printProgress((double)counter / (es.size() * spacings.size()));
    //     }
    // }
    // // write Data to csv
    // string headers = "Eccentricity,Spacing,Fevals,Hmax,Prepare Segmentation,Forward Satellite,Aftward Satellite,Bootstrapped Satellite";
    ...
    // }
    // myfile.close();
    // filename = "INT_bootstrap_no_bootstrap_tradestudy_Earth.csv";
    // myfile.open(filename);
    // myfile << headers<<"\n";
    // myfile << std::setprecision(15);
    // for (auto const &x : DataNoBootstrap)
    // {
    //     myfile << x.first.first << "," << x.first.second << "," << x.second.TotalFevals << "," << x.second.Hmax << "," << x.second.PrepFevals << "," << x.second.ForPIFevals << "," << x.second.AftPIFevals << "," << x.second.BootstrapFevals << "\n";
    // }
    // myfile.close();
