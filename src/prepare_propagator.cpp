...
void prepare_propagator(Orbit &orbit)
{
  ...
  vector<double>&T2 = orbit.T2; // [(M+1)x(N+1)]
  vector<double>&P2 = orbit.P2; // [(N+1)xN]
  vector<double>&T1 = orbit.T1; // [(M+1)xN]
  vector<double>&P1 = orbit.P1; // [Nx(N-1)]
  vector<double>&Ta = orbit.Ta; // [(M+1)x(N-1)]
  vector<double>&A = orbit.A;   // [(N-1)x(M+1)]
  ...
  T2.resize((M + 1) * (N + 1), 0.0); // [(M+1)x(N+1)]
  P2.resize((N + 1) * N, 0.0);       // [(N+1)xN]
  T1.resize((M + 1) * N, 0.0);       // [(M+1)xN]
  P1.resize(N * (N - 1), 0.0);       // [Nx(N-1)]
  Ta.resize((M + 1) * (N - 1), 0.0); // [(M+1)x(N-1)]
  A.resize((N - 1) * (M + 1), 0.0);  // [(N-1)x(M+1)]
  t_orig.resize(orbit.seg + 1, 0.0);
  tvec.resize(orbit.seg + 1, 0.0);
