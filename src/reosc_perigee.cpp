...
void reosc_perigee(double tf, Orbit &orbit)
{
  ...
  vector<double> &times = orbit.times_seg;
  ...
  // FIXME: This if statement only catches the last segment of each orbit if the value of k is properly set at the start of the orbit. For the first orbit this is not the case if the initial
  // state is not at perigee. A more robust detection of perigee passage is needed to ensure that the first orbit is properly re-osculated. Potentially could be sovled by figuring out which segment
  // the initial state is in and then setting k to that value.
      ...
      for (int i = 1; i <= M + 1; i++)
      {
        ...
        if ((i > 2) && (fvec[i - 1] < fvec[i - 2]))
        {
          ...
          break;
           ...
           //print debug message for reosculating including the eccentricity and new 
           if (g_DEBUG_PICARD)
           {
             cout << "Reosculating perigee." << std::endl;
             cout << "Eccentricity: " << e << std::endl;
             cout << "New perigee: " << f_new << std::endl;
             cout << "New time: " << tf << std::endl;
           }
    }
